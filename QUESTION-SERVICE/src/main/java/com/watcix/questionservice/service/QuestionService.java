package com.watcix.questionservice.service;
...
import com.watcix.questionservice.repository.QuestionRepository;
...
public class QuestionService {
    ...
    public ResponseEntity<List<QuestionDto>> getQuestionsFromId(List<Integer> questionIds) {
        ...
        for(Question question : questions){
            QuestionDto wrapper = new QuestionDto();
    ...
    public ResponseEntity<ResultDto> getScore(List<ResponseDto> responses) {
        ResultDto resultDto = new ResultDto();
        int right = 0;

        for(ResponseDto response : responses){
            Question question = questionRepository.findById(response.getId()).get();
            if(response.getOption().equals(question.getRightAnswer())) {
                right++;
        ...
        resultDto.setTotalScore(right);
        resultDto.setResponseDtoList(responses);
